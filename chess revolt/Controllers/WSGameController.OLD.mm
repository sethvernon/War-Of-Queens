...
#import "WSGameController.h"
#import "EngineController.h"
//#import "GameController.h" replaceing this with WSGameController SAV 1.27.14
...
@interface WSView()
@property (nonatomic, readwrite)WSGame *wsGame;

@end

@implementation WSGameController

@synthesize wsGame = _wsGame; /// replaced original synthesized to _ because these are very likeley to have setter and getter methods.
@synthesize pieceViews = _pieceViews;
@synthesize pieceImages = _pieceImages;
@synthesize boardView = _boardView;



/////*** Added Lazy instantiation to objects for proper Objective-C Syntax  SAV 1.27.14 ****////////

- (WSGame *)wsGame
{
    if (!_wsGame)
        _wsGame = [[WSGame alloc] init];
    return _wsGame;
}
...
{
    if (!_pieceImages)
        _pieceImages = [[NSArray alloc] init];
    return _pieceImages;
}
...
- (instanceType)initWithWSView:(WSView *)bv      /////    replaced (boardView *) for new UIView Subclass  SAV 1.27.14
           moveListView:(MoveListView *)mlv
           analysisView:(UILabel *)av   //////// might replace these with storyboard objs //
          bookMovesView:(UILabel *)bmv
         whiteClockView:(UILabel *)wcv
         blackClockView:(UILabel *)bcv
        searchStatsView:(UILabel *)ssv {
    if (self == [super init]) {
        _boardView = bv;   ///// _boardView points to WSView ///// SAV 1.27.14
        _moveListView = mlv;
        _analysisView = av;
        _bookMovesView = bmv;
        _whiteClockView = wcv;
        _blackClockView = bcv;
        _searchStatsView = ssv;
        
        NSLog(@"Loaded initWithBoardView");
        
//        _game = [[Game alloc] initWithGameController: self]; /// replace this...actually this is already replaced in my lasy instantiation I believe.
...
- (Square)rotateSquare:(Square)sq {
    ...
    NSLog(@"loaded engineController and ran the rotateSquare method");
}
