...
//    *istreamptr  = [NSMakeCollectable(readStream)autorelease]; //// removed garbage collection reference for ARC /// Seth 1.2.14
...
- (void)connectToServer:(NSString *)serverName port:(int)portNumber {
   ...
   NSLog(@"host name: %@", [site host]);
    ...
    NSInputStream *itemp;  // Added temparary local variable for NSInputStream // Seth 1/3/14
    NSOutputStream *otemp;  ////*****                    **** ///////
    
   [NSStream getStreamsToHostWithName: serverName
                                 port: portNumber
                          inputStream: &itemp     ///// temporary local variable /////
                         outputStream: &otemp];
    
    istream = itemp; //// assigned external CFInputStream to local variable // Seth 1/3/14
    ostream = otemp; /////*****                           *******////////

   if (istream == nil)
      NSLog(@"Error 2");

    if (istream) {              ///// replaced with if statement //// Seth 1.2.14
   [istream setDelegate: self];
   [istream scheduleInRunLoop: [NSRunLoop currentRunLoop]
                      forMode: NSDefaultRunLoopMode];
   [istream open];
    }
    if (ostream) {                            ///// replaced with if statement //// Seth 1.2.14
...
- (void)stream:(NSStream *)stream handleEvent:(NSStreamEvent)eventCode {
   if (eventCode == NSStreamEventHasBytesAvailable) {
      ...
      for (id line in lines) {
       ...
       str = nil;  /// in place of release /// Seth 1.2.14
       data = nil;  /// in place of release /// Seth 1.2.14
