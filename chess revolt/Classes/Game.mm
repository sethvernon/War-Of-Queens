...
- (id)initWithGameController:(GameController *)gc PGNString:(NSString *)string {    //// SAV removed PGN because this is a  network start... I believe. 2.20.14
    
    if (!gc) gc = [[GameController alloc] init];
    
    // aloc init instead of init with game controller. it might not work. be sure to test.
...
- (id)initWithGameController: (GameController *)gc {
   return [self initWithGameController: gc
                                   FEN: @"rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1"];  /// changing the inital position of the pieces.
}
